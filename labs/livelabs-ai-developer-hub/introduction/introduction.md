...
The LiveLabs AI Developer Hub brings together automation-first workflows that help authors move faster without lowering quality. Instead of treating workshop development as a chain of manual tasks, the hub shows how to set up Codex, install reusable skills, generate a LiveStack demo, draft workshops from source material, capture screenshots, add knowledge checks, and adapt content for new audiences.

This matters because the bottleneck in workshop delivery is rarely markdown alone. It is the surrounding work: planning, refactoring, image capture, QA cleanup, demo generation, and keeping versions aligned. The hub packages those workflows so authors can spend more time shaping outcomes and less time repeating setup steps.

## Introduction

This workshop starts by preparing a development machine for Oracle LiveLabs work. You will install Codex, add reusable Oracle AI Developer skills, and verify that Codex can find them. After that foundation is in place, you can optionally use LiveStacks Orchestrator with Podman to generate and run a local Oracle-first demo. If the instructor provides a prebuilt package, you can instead run a supplied zip on an OCI Compute instance.
...
> **Important:** Review AI-generated content carefully for accuracy, completeness, and alignment with **Oracle** and **LiveLabs** standards. Codex can accelerate development, but it may produce incomplete or incorrect output. Authors are responsible for validating technical content, testing instructions, and ensuring overall quality before publishing.
