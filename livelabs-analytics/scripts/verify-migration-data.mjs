...
const payloadPath = path.resolve(projectRoot, valueFor("--payload", "data/portfolio_inventory.json"));
const htmlPath = path.resolve(projectRoot, valueFor("--html", "index.html"));
const outputDir = path.resolve(valueFor("--output-dir", path.join(projectRoot, ".migration-audit")));
...
const replacementVerification = {
  ...
  checks: {
    ...
    singularVisibleRankedTables: ["at-risk-top-100-workshops", "at-risk-top-100-sprints", "retire-now-top-100-workshops", "retire-now-top-100-sprints"].every((id) => (html.match(new RegExp(`data-filter-table="${id}"`, "g")) || []).length === 1),
  },
