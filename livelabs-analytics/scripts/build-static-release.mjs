...
const output = path.resolve(root, outputFlag >= 0 ? args[outputFlag + 1] : "release");
...
try {
  ...
  const inventoryData = JSON.parse(fs.readFileSync(path.join(staging, "data/portfolio_inventory.json"), "utf8"));
  manifest.data_snapshot = {
    ...(manifest.data_snapshot || {}),
    portfolio_inventory_generated_at: inventoryData.metadata?.generated_at || null,
    canonical_portfolio_payload: "data/portfolio_inventory.json",
  };
