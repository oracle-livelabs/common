...
const releaseRoot = path.resolve(projectRoot, valueFor("--release", "dev/releases/local"));
...
const inventoryPayload = readJson(path.join(releaseRoot, "inventory/data/portfolio_inventory.json"));
...
const fragmentDirectory = path.join(releaseRoot, "assets", "fragments");
const dashboardFragments = fs.existsSync(fragmentDirectory)
  ? fs.readdirSync(fragmentDirectory).filter((file) => file.endsWith(".html")).sort().map((file) => fs.readFileSync(path.join(fragmentDirectory, file), "utf8"))
  : [];
const dashboardSourceBundle = [
  html,
  stylesheet,
  fs.readFileSync(path.join(releaseRoot, "assets", "js", "dashboard.js"), "utf8"),
  ...dashboardFragments,
].join("\n");
...
const emptyTables = tableIds.filter((id) => {
  const table = dashboardSourceBundle.match(new RegExp(`<table[^>]*\\bid="${id}"[\\s\\S]*?<\\/table>`, "i"))?.[0] || "";
...
const duplicateCutHeadings = [...dashboardSourceBundle.matchAll(/<h[23][^>]*>([\s\S]*?)<\/h[23]>/gi)]
...
const hasOracleContacts = /[A-Z0-9._%+-]+@oracle\.com/i.test(dashboardSourceBundle)
