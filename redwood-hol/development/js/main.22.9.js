...
let main = function() {
    ...
    let setupContentNav = function(manifestFileContent, articleElement) {        
            ...
            if (hide_expand_button ==  "true" || hide_expand_button == "yes" || $(articleElement).find('h2').length <= 2) {
                $('#btn_toggle').hide();
            }
    ...
    let performQA = function(articleElement, markdownContent, manifestFileContent) {
        ...
        const more_info = "Please see <a href='https://livelabs.oracle.com/cdn/common/sample-livelabs-templates/create-labs/labs/workshops/freetier/?lab=4-labs-markdown-develop-content' target='_blank'>using the LiveLabs template</a> for more information.";
...
let download = function() {
    ...
    $.when($('img').each(function() {
        ...
        download_file($('.selected span').text().replace(/[^[A-Za-z0-9:?]+?/g, '') + '.html', '<html><head><link rel="stylesheet" href="https://livelabs.oracle.com/cdn/common/redwood-hol/css/style.min.css" /><link rel="shortcut icon" href="https://livelabs.oracle.com/cdn/common/redwood-hol/img/favicon.ico" /></head><body style="padding-top: 0px;">' + $('#contentBox')[0].outerHTML + '</body></html>');
    });
}
