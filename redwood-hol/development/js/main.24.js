...
let main = function () {
    ...
    let convertQuizBlocks = function (markdown) {
        let quizRegex = /`{3,}quiz\s*\n([\s\S]*?)`{3,}/g;
        let quizId = 0;

        return markdown.replace(quizRegex, function (match, content) {
            let html = '';
            ...
            questions.forEach(function (q, qIndex) {
                ...
                html += '<div class="ll-quiz" data-quiz-id="' + qId + '">';
    ...
    const allowCodeCopy = function (articleElement) {
        ...
        $(articleElement).find('.copy-button').click(function () {
            ...
            const copyText = $(this).next().find('.copy-code').map(function () {
