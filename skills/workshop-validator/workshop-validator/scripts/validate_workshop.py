...
def collect_markdown_files(root: Path) -> List[Path]:
    return sorted(
        p for p in root.rglob("*.md") if p.name != "VALIDATION-RESULT.md"
    )
...
def check_markdown_rules(path: Path, lines: Sequence[str]) -> List[str]:
    ...
    # H1 count ignoring fenced code blocks
    h1_count = 0
    in_code = False
    ...
    for match in IMAGE_ALT_EMPTY_RE.finditer("\n".join(lines)):
    ...
    for idx, line in enumerate(lines, start=1):
        if line.startswith("## Task") and not TASK_HEADER_RE.match(line):
            errors.append(
                f"Line {idx}: Task headers must match '## Task N: Description'."
            )
    ...
    if copy_opens != copy_closes:
        errors.append(
            f"Mismatched <copy> tags (open: {copy_opens}, close: {copy_closes})."
        )
    ...
    for pos, start in enumerate(task_indices):
        ...
        if block and not any(re.match(r"\s*\d+\. ", ln) for ln in block):
            errors.append(
                f"Line {start+1}: Task sections must contain numbered steps inside the task."
            )
        for offset, ln in enumerate(block):
            ...
            if stripped.startswith("```") and indent < 4:
                errors.append(
                    f"Line {line_no}: Code blocks inside tasks must be indented four spaces."
                )
            if stripped.startswith("![") and indent < 4:
                errors.append(
...
def analyze_lanham(lines: Sequence[str]) -> (int, List[str]):
    ...
    if long_sentences:
        notes.append(
            f"{len(long_sentences)} sentence(s) exceed 20 words; tighten wording."
        )
        score -= 1
    if passive_hits:
...
def validate_structure(root: Path) -> StructureReport:
    ...
    variant_dirs = [p for p in workshops_dir.iterdir() if p.is_dir()]
    if not variant_dirs:
        report.missing_items.append(
            "No workshop deployment variants found inside 'workshops/'."
        )
    ...
    # Lab folders = directories at root excluding 'workshops' and hidden entries
    for entry in root.iterdir():
        ...
        if not canonical_md.is_file():
            report.missing_items.append(
                f"{entry.relative_to(root)} missing {entry.name}.md file."
            )
...
def build_report(result: ValidationResult, root: Path) -> str:
    ...
    for file_report in result.files:
        ...
        lines.append(
            f"- ✍️ Lanham score: {file_report.lanham_score}/5"
        )
