...
```text
Install `redwood-creator-slim` skill into my local Codex skills directory. Inspect `SKILL.md`, use the embedded `name:` `redwood-creator` as the installed folder name, ignore `__MACOSX`, `__pycache__`, and `*.pyc`, and verify the installed copy after copying.
```
