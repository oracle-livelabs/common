...
## Quick Start
- Gather source links and artifacts from the user: ideas, blogs, product pages, docs, web pages, diagrams, commands, screenshots, Jupyter notebooks, slide decks, PowerPoint files, and sample SQL.
- Classify every source as Oracle-owned/internal, external/non-Oracle, or unclear before drafting. Always track provenance; require attribution and rights review for external or unclear sources.
- For source bundles such as DOCX, PPTX, Pages, PDF, ZIP, notebooks, or exported sites, inspect embedded assets separately from the parent file. Classify images, screenshots, diagrams, logos, charts, datasets, code, quoted passages, and linked media on their own.
- Notify the user when external or unclear sources, images, diagrams, copied datasets, or adapted code require attribution. If all sources are Oracle-owned or Oracle-authored, still record provenance but do not raise a third-party attribution warning.
- Stop before planning or authoring from external/non-Oracle or unclear source material until the workshop author explicitly confirms, in the current build request, that they have the source owner's approval to build a workshop from that material. Do not rely on approval from earlier turns, prior runs, memory, or existing traceability files. Record the current approval in `TRACEABILITY.md`. After confirmation, learner-facing acknowledgements must include: `Built with permission from the author(s).`
...
- Create a source traceability stub when the workshop is based on outside material or any source-backed Oracle material:
...
- Map claims, commands, images, diagrams, datasets, and adapted code back to the source material.
- Do not assume a parent document's ownership covers embedded assets. If an embedded asset's origin is not clear, classify it as unclear and apply the external-source approval gate before using or recreating it.
- Record each source as Oracle-owned/internal, external/non-Oracle, or unclear. Treat partner-authored, customer-authored, third-party-hosted, or ambiguous material as unclear until the user confirms ownership and usage rights.
- For external/non-Oracle or unclear sources, including embedded assets inside otherwise Oracle-owned bundles, stop the workflow before creating the lab plan, scaffold, markdown, images, or derived artifacts. Ask the workshop author to confirm they have approval from the source owner or author to build a workshop from the material for this specific build. Continue only after explicit confirmation in the current user request or response. After confirmation, use `Built with permission from the author(s).` in learner-facing acknowledgements for the external source.
- Keep Oracle-owned/internal sources in traceability for auditability, but do not add public-style attribution unless the source or user requires it. Do not expose confidential internal links, author names, or source details in learner-facing labs without approval.
- Add learner-facing source acknowledgements only for external-facing public URLs. If that source is external/non-Oracle and permission was confirmed, use `* **Source** - [Source title](https://public-url). Built with permission from the author(s).`
- Do not list Oracle-internal, Oracle-owned private, local, SharePoint, iCloud, unpublished, or confidential sources in learner-facing acknowledgements. Record them only in `TRACEABILITY.md` when provenance is needed.
...
- Use:
  ...
  - `references/source_traceability.md` when any source material, internal or external, drives the workshop
...
- Resolve failures in this order:
 ...
 4a. If the user asked for an embed, confirm the final lab contains an iframe-based embed rather than a launch-only affordance.
  5. Time fields, acknowledgements, source attribution notes, embedded-asset provenance, and recorded external-source approval when applicable.
...
- Return:
  ...
  - workshop outline,
  - QA summary,
  - attribution, source-provenance, embedded-asset provenance, and external-source approval summary,
  - unresolved SME gaps.
...
## Scripts
- `scripts/setup_or_upgrade_skill.sh` to mark this release as `26.5.3`, scan for older author-skill installs, and optionally archive them as `26.5`.
