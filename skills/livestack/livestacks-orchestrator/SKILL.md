...
Every generated app must also satisfy the **Scene Experience Contract**. Scenes cannot be clones of one generic stage with different labels. Each scene should own:

- a scene-specific component or page module such as a triage board, API workbench, report runner, schema lineage inspector, architecture comparator, governed action publisher, map/graph, chat/workbench, or dataset manager
- a distinct interaction pattern with controls the operator actually uses
- scene-local state such as selected records, filters, result rows, payloads, report keys, map selections, or action choices
- visible domain objects tied to the business scenario, not abstract placeholder cards
- real Oracle evidence per scene, including route, SQL/PLSQL, view/table/model/result, score, payload, or dataset state
- workflow handoffs to the next scene or operator step
- screenshot inventory captions that prove scenes are visually and behaviorally different

The default scaffold and A+ grader must reject a repeated `scene-stage` / single `runPrimaryAction` pattern even when that shell has Oracle-branded chrome.

`Oracle Internals` must be a scene-owned technical exhibit, not a generic metadata panel. Every scene should register or render its own:

- short `What's Happening` explanation tied to the visible operator action
- color-coded Oracle capability badges
- real SQL or PL/SQL snippets that could run against the bundle's Oracle schema
- diagram boxes that show route, package, table/view, model, or provider data flow
- scene-specific security/governance callout
- live evidence or dynamic state such as health, selected record, active filters, report key, dataset state, route, score, or returned payload status
- AI provider boundary and data-egress caveat when AI or an LLM is in scope

The `Use Your Own Data` overlay must also own an Oracle Internals state. When the dataset manager is open, the rail should explain validate, upload/replace, restore-demo, active dataset state, job status, and the admin authorization boundary instead of continuing to show the prior scene's static content.
...
- Treat first-iteration app quality as a product requirement: the opening screen must be an interactive capability-led operator workflow, not a static welcome page, generic overview dashboard, or scaffold reminder.
- Treat `Use Your Own Data` as a first-iteration acceptance requirement whenever demo data is replaceable. It must be visible in the first app iteration and wired as a real operator dataset flow, not deferred to documentation.
...
- Make Oracle AI Database 26ai feature strategy the first design decision. Use `$oracle-db-skills` to select the minimum credible Oracle-native feature set and usage pattern before shaping the UI scene architecture.
...
- Derive dynamic scene architecture from the selected Oracle feature usage patterns, operator decision points, business story complexity, evidence needs, guide/demo pacing, and customer-data onboarding needs. Do not default to one fixed scene count.
- Default sparse briefs to an `operator_workbench` first iteration unless the working PRD justifies `converged_showcase` or `hybrid`. The app structure should include a Command Center, Data Foundation, and the number of capability-backed workflow scenes needed to prove the chosen Oracle 26ai features. The first screen must open on a concrete operator job, with a primary CTA that changes state and an Oracle evidence control that explains the current scene.
...
- For Podman LiveStacks, make `compose.yml` follow the capability-led runtime core contract: use hard-coded published ports in the compose file rather than env-overridable port mappings, declare top-level `networks.default`, and give every service an explicit `hostname` plus matching `networks.default.aliases` entry.
...
- Treat the repeatable LiveStack shell anatomy as part of that UI acceptance bar: an Oracle logo/name lockup on the left, a left-side scene rail using known-rendering `oj-fwk-icon-*` glyphs, scene entries that show the Oracle features demonstrated in that scene, a top-right `Use Your Own Data` utility, and a right-side collapsible Oracle Internals rail populated by the active scene.
- Drive the generated shell from a scene metadata manifest. The manifest should define scene id, label, title, navigation glyph, Oracle feature tags, primary CTA, guide/screenshot naming, and Oracle Internals evidence payload so future LiveStacks repeat the same polished structure without copying an industry app raw.
...
- When no target app framework is specified, start from the capability-led operator application pattern and apply overlays for industry vocabulary, pain-point workflow, dynamic scene architecture, Oracle capability mapping, data/import contract, and guide/runbook content. The default runtime remains one Node.js / Express app service that serves a built Oracle JET / Redwood frontend shell from the same `app` container while honoring the fixed port, ORDS-first, and Oracle-evidence requirements. The default frontend must not include Tailwind.
...
- When demo data is meant to be replaceable, include an operator-only dataset administration flow from the capability-led operator application core: one persistent top-right masthead utility labeled `Use Your Own Data` that opens an overlay dataset manager, plus template download, validate-only preview, destructive upload or replace, active Oracle-backed dataset state, job status, and restore-demo. Only deviate from that trigger pattern when the user explicitly asks for a different dataset-entry experience.
- Destructive dataset routes such as upload, replace, restore-demo, reseed, and generated admin actions must fail closed behind `ADMIN_TOKEN`, `Authorization`, CSRF, JWT, or an equivalent operator-admin boundary. A visible `Use Your Own Data` control is incomplete unless the runtime also exposes template, validate, upload, restore-demo validate, restore-demo execute, active dataset state, and job-status routes.
...
- Use the public healthcare guide as the richest production guide baseline, with public finance/retail guides as cross-checks. Use local checkouts only as faster mirrors when they exist. Mirror their structure, cadence, screenshot-backed task style, and runbook tone before applying solution-specific names, ports, assets, and labels.
...
- Require `guide/workshops/desktop`, `guide/workshops/sandbox`, and `guide/workshops/tenancy` for every production guide generated by this orchestrator. Accept both paired `<copy>` command markers and wrapped `<copy>...</copy>` command markers.
...
- Include a full `Use Your Own Data` guide scene whenever demo data is replaceable. Cover the dataset tool, active dataset state, template ZIP, completed ZIP selection, validation, upload/replace, restore-demo, job/status behavior, and data-safety expectations.
- Capture guide screenshots from the real running app with automated browser tooling, store the capture inventory under `output/guide-screenshots/`, and integrate selected callout/highlight images into `guide/**/images` rather than using mockups or aspirational placeholders.
- Require one app-visible Oracle evidence surface: a persistent clickable/collapsible, scene-aware `Oracle Internals` control or a dedicated database X-Ray mode with equivalent interaction depth. It must be populated by the active scene and show how Oracle powers the current screen through real ORDS routes, SQL or PL/SQL activity, Oracle feature usage, AI/vector/profile boundaries, or request-flow evidence rather than generic marketing copy.
...
3. Normalize the business input into an explicit brief and synthesize `input/working-prd.md` using `references/prd-build-contract.md`.
   ...
   - Record `oracle_26ai_primary_capability`, `oracle_26ai_secondary_capabilities`, `feature_usage_pattern`, `why_this_oracle_feature_fits`, dynamic scene architecture, the chosen story mode, `scene_count_target`, `primary_user_loop`, `primary_cta_path`, `first_scene_goal`, `first_interaction`, `first_decision_point`, `first_oracle_evidence`, `evidence_surface_per_scene`, `oracle_internals_scene_payload`, `upload_your_own_data`, `redwood_jet_ui_quality_bar`, `ai_capability_mode`, `provider_boundary`, `data_egress_caveat`, and feature-to-scene mapping in the working PRD before the UI/UX and full-stack roles begin detailed execution.
...
9. Materialize the bundle:
   - Use `python3 scripts/init_livestack_bundle.py <output-dir> <solution-slug>` if a workspace folder does not already exist. The scaffold must carry `input/template-provenance.json` and then be adapted from the capability-led operator application pattern plus overlays before it can be called production-ready.
   - Record the selected overlay layers in `input/template-provenance.json`: industry vocabulary, pain-point workflow, dynamic scene architecture, Oracle capability map, data contract, and guide runbook.
   ...
   - Keep `stack/compose.yml` and `stack/.env.example` byte-for-byte aligned to the capability-led runtime baseline unless the user explicitly approves a runtime-contract exception. Put domain-specific app/schema/ORDS/story choices in application code, database scripts, seed/config data, docs, guide content, and `input/template-provenance.json` instead.
   ...
   - Run `python3 scripts/grade_livestack_bundle.py <solution-root>` and do not pass the bundle unless it gets `A+` with capability-led operator application evidence, golden-core runtime parity, and red/green test evidence.
10. Author the LiveStack guide:
   ...
   - Use the public healthcare guide first for the full production guide shape, then use public finance and retail guides to avoid overfitting the domain prose. If matching local checkouts exist, use them only as faster mirrors of those public references.
   ...
   - Add a complete Use Your Own Data scene for generated replaceable-data LiveStacks, with screenshots for the top-bar control, template/download path, validation/upload path, restore-demo path, and active dataset state.
...
- dynamic scene architecture with a recommended story mode and right-sized scene count derived from Oracle feature breadth, operator decision points, business story complexity, evidence needs, and guide/demo pacing
...
- `guide/` workshop tree authored as a LiveStack demo runbook, with introduction, screenshot-backed scene labs, a full Use Your Own Data scene, local-run lab, and desktop/sandbox/tenancy workshop manifests
- persistent clickable/collapsible, scene-aware Oracle Internals experience or equivalent database X-Ray mode
