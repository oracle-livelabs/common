...
- Reworked the default React scaffold away from a repeated `scene-stage` into Command Center, Data Foundation, capability workflow, and operator-action components with separate state, controls, domain objects, evidence, and handoffs.
- Strengthened semantic validation and A+ grading so a generic repeated scene-stage with one shared primary action cannot pass even when Oracle chrome, scene manifest metadata, and Oracle Internals are present.
- Added screenshot-inventory caption checks so repeated generic captions cannot satisfy the guide evidence gate.

## 0.1.0-preview.19 - 2026-05-27

- Promoted the richer healthcare/PM Collective Oracle Internals pattern into the generated app contract: per-scene `What's Happening`, capability badges, SQL/PLSQL, architecture/data-flow boxes, governance callouts, and live evidence.
- Updated the default React scaffold to emit `OracleInternalsContent.jsx`, richer scene-manifest internals metadata, and dataset-manager-specific Oracle Internals state when `Use Your Own Data` is open.
- Strengthened semantic validation and A+ grading so shallow Oracle Internals rails fail even when they are clickable, collapsible, and scene-aware.
- Fixed vector/RAG claim detection so standalone `RAG` is matched as a feature claim without false positives from words such as `dragging` or `storage`.

## 0.1.0-preview.18 - 2026-05-20

- Promoted the healthcare LiveStack shell pattern into the default generated UX contract: Oracle logo/name lockup, left-side JET-glyph scene rail, per-scene Oracle feature chips, top-right `Use Your Own Data`, and a right-side collapsible scene-aware `Oracle Internals` rail.
- Added a generated scene manifest pattern so navigation, guide order, screenshot naming, Oracle feature tags, primary actions, and Oracle evidence payloads are driven from reusable scene metadata instead of ad hoc page wiring.
- Strengthened semantic validation and A+ grading to reject generic centered shells that omit the left rail, scene manifest, feature chips, Oracle lockup, top-right dataset entry, or active-scene Oracle Internals registration.
- Added a healthcare overlay example for repeatable provider-network vocabulary while preserving the strict four-service, fixed-port, ORDS-first, no-Tailwind runtime contract.

## 0.1.0-preview.17 - 2026-05-18

- Reframed the app standard as a capability-led operator application: Oracle AI Database 26ai feature usage patterns are selected first, then expressed through Command Center, Data Foundation, and dynamic capability-backed scenes.
- Required `Oracle Internals` to be a persistent clickable/collapsible, scene-aware evidence control populated by the active scene rather than a static documentation panel.
- Updated working-PRD, overlay, UI concept, Oracle capability map, scaffold, semantic validation, and A+ grading gates around `oracle_26ai_primary_capability`, `feature_usage_pattern`, `evidence_surface_per_scene`, and `oracle_internals_scene_payload`.
- Updated the default React starter to include Command Center, Data Foundation, dynamic capability-backed workflow scenes, top-right `Use Your Own Data`, and a collapsible active-scene Oracle Internals panel.

## 0.1.0-preview.16 - 2026-05-13

- Reworked `scripts/self_update.py` to use a zip-manifest update flow instead of sparse-cloning the expanded GitHub skill directory.
- The updater now checks `skills/livestack/livestacks-orchestrator.update.json`, compares the manifest `content_hash` to the installed skill, and downloads `livestacks-orchestrator.zip` only when the content hash differs.
- Added archive SHA-256 verification and extracted-package content-hash validation before replacing the installed skill.
- Removed the `git` requirement from the self-update path; only `python3` and network access to the manifest/archive are required for automatic updates.
...
- Improved bundled `livestack-guide-builder` low-friction behavior: desktop manifests now prioritize local download/run instructions, scaffolded guide dates default to the current date, and public reference guide URLs are documented with local checkouts treated only as optional mirrors.
