...
- start from the best-fit Oracle AI Database 26ai feature usage pattern for the business pain point
- name `oracle_26ai_primary_capability`, `oracle_26ai_secondary_capabilities`, `feature_usage_pattern`, and `why_this_oracle_feature_fits`
- classify the solution into a story mode such as `operator_workbench`, `converged_showcase`, or `hybrid`
- justify the dynamic scene architecture and recommended `scene_count_target` from the workflow, operator decision points, inferred Oracle feature breadth, evidence needs, and guide/demo pacing
...
- every generated app should include a visible AI capability unless the working PRD explicitly rejects AI with a documented reason; Oracle Internals must be persistent, clickable/collapsible, scene-aware, populated by the active scene, and show the chosen AI mode, Oracle feature, model/profile boundary, source attribution where relevant, and data-egress caveat
...
- a persistent top-right masthead utility labeled `Use Your Own Data` as the default operator entry point in the application chrome
...
- Treat Oracle 26ai capability strategy as the first design decision, ahead of UI scene labels. The application shell exists to make the chosen feature usage pattern operational and inspectable.
...
- Map every selected Oracle feature to at least one operator action, one capability-backed scene, and one Oracle Internals scene payload.
...
## Capability-Led Operator Application Pattern

Default LiveStacks are capability-led operator applications. The domain can change, but the application should keep this pattern:

- Command Center as the primary operator entry, with a real decision or action rather than a static landing page
- Data Foundation as an early scene that explains the active dataset, import contract, derived artifacts, and Oracle-backed state
- dynamically generated capability-backed workflow scenes; do not force a fixed scene count
- optional Ask Data, Agent Console, or AI assistant scenes only when the selected Oracle 26ai feature usage pattern justifies them
- persistent top-right `Use Your Own Data`
- persistent clickable/collapsible, scene-aware right-side Oracle Internals populated by the active scene
- dataset-manager-specific Oracle Internals content when the `Use Your Own Data` overlay is open
- Oracle logo/name lockup on the left
- left-side scene navigation using Oracle JET `oj-fwk-icon-*` glyphs
- per-scene Oracle feature chips or tags that show which Oracle capabilities will be demonstrated
- scene manifest metadata for scene id, label, title, navigation glyph, Oracle feature tags, primary CTA, guide/screenshot name, scene type, interaction pattern, evidence object, handoff targets, and Oracle Internals payload
- Scene Experience Contract evidence: scene-specific component/page module, distinct interaction pattern, scene-local state, visible domain objects, real Oracle evidence per scene, workflow handoffs, and screenshot captions that prove visual and behavioral distinction

## Golden Core And Overlay Pattern

Default LiveStacks are generated from the capability-led operator application pattern plus overlays, not from a raw industry folder.

- The capability-led runtime core owns runtime topology, fixed ports, ORDS-first API flow, `/healthz`, dataset-admin shape, Oracle Internals, Oracle JET / Redwood styling, Oracle Sans, and JET glyph iconography.
- The capability-led UI core owns the repeatable shell anatomy: Oracle logo/name lockup, left-side scene rail, top-right utility controls, per-scene Oracle feature chips, right-side Oracle Internals, the scene manifest, and the Scene Experience Contract that prevents repeated generic scene stages.
- The capability-led runtime core also owns `stack/compose.yml` and `stack/.env.example`. Those files must stay the same across generated default LiveStacks; do not use them for industry, customer, story, schema, app-image, or ORDS-module variation.
- Industry overlays such as `highered`, `lifesciences`, `media`, and `sled` in `assets/template-overlays/industry-overlays.json` are examples. They should inform labels, scenes, semantic views, seeded vocabulary, prompts, and guide wording.
...
- Apply overlays in this order: Oracle 26ai capability strategy, feature usage pattern, industry vocabulary, pain-point workflow, dynamic scene architecture, Oracle capability map, data contract, guide runbook.
...
- a persistent clickable/collapsible, scene-aware Oracle Internals control, or a dedicated database X-Ray mode with equivalent interaction depth
...
- per-scene technical exhibit sections: `What's Happening`, capability badges, SQL/PLSQL, architecture flow, security/governance, and live evidence
...
- Include introduction, local-run/download, screenshot-backed per-scene labs, a full Use Your Own Data scene when demo data is replaceable, and desktop, sandbox, and tenancy workshop manifests. A separate conclusion lab is optional when the app already has a real final workflow scene.
...
- Capture screenshots from the real running app with installed automated browser tooling when available and integrate selected callout/highlight images into the relevant `guide/**/images/` folders.
...
When the user does not specify an application framework, the default generated scaffold should instantiate the capability-led operator application baseline and then apply domain overlays:
...
- a premium capability-led operator workbench starter rather than a static welcome page or generic overview dashboard
- Command Center, Data Foundation, and right-sized capability-backed workflow scenes
...
The default scaffold is only a starting point. It is not production-ready until the bundle replaces starter runtime code with domain-specific ORDS-backed flows, a real scene-aware Oracle evidence surface, a working `Use Your Own Data` dataset manager when demo data is replaceable, an automated database bootstrap path where database artifacts exist, and screenshot-backed guide evidence.
...
<solution-slug>/
  ...
  guide/
    ...
    scene-01-<scene-slug>/
      ...
      images/
        <capture>.png
    conclusion/                  # optional when a real final scene exists
      conclusion.md
    workshops/
      desktop/
        index.html
...
- `guide/scene-*/scene-*.md`: includes a Use Your Own Data or dataset workflow scene when demo data is replaceable
- `guide/conclusion/conclusion.md`: optional closing recap and next steps when there is no real final app scene
...
- The bundle declares a story mode, a dynamic scene architecture, a right-sized `scene_count_target`, and a primary CTA path rather than only a static screen inventory.
- The bundle declares `oracle_26ai_primary_capability`, `oracle_26ai_secondary_capabilities`, `feature_usage_pattern`, `why_this_oracle_feature_fits`, `evidence_surface_per_scene`, and `oracle_internals_scene_payload`.
...
- Each scene lab includes real app screenshots, with callouts, highlights, or tight crops for controls and evidence that the user clicks or reviews.
...
- If demo data is replaceable, the bundle includes the dataset admin flow with the default top-right `Use Your Own Data` masthead utility plus template download, validate-only preview, upload or replace, active dataset state, and demo restore, unless the user explicitly requested another entry pattern.
- If demo data is replaceable, `Use Your Own Data` is working in the first app iteration and covered by a full guide scene with screenshots for the dataset manager, template download, validation/upload, restore-demo, and active dataset state; it is not only a documented future path.
- The application includes a credible persistent clickable/collapsible, scene-aware Oracle Internals control or equivalent database X-Ray mode tied to real runtime behavior, with scene-specific SQL/PLSQL, architecture diagrams, governance callouts, and live evidence rather than generic copy.
- The application includes scene-specific components/page modules, distinct interaction patterns, scene-local state, visible domain objects, real Oracle evidence per scene, workflow handoffs, and screenshot captions proving the scenes are visually and behaviorally different.
