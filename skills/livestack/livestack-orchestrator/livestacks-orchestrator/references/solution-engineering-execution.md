...
- Oracle 26ai primary/secondary capability strategy and feature usage pattern
- dynamic scene architecture derived from capability breadth and operator decisions
- `Use Your Own Data` acceptance criteria when demo data is replaceable
...
7. Define `oracle_26ai_primary_capability`, `oracle_26ai_secondary_capabilities`, `feature_usage_pattern`, and `why_this_oracle_feature_fits`.
8. Define dynamic scene architecture: Command Center, Data Foundation, and the right-sized set of capability-backed workflow scenes. Do not force a fixed scene count.
9. Define `scene_count_target`, `primary_user_loop`, `primary_cta_path`, `first_scene_goal`, `first_interaction`, `first_decision_point`, and `first_oracle_evidence`.
...
- pain-point classification
- core workflow
- story architecture with `story_mode`, dynamic scene architecture, right-sized `scene_count_target`, `primary_user_loop`, and `primary_cta_path`
- capability strategy with `oracle_26ai_primary_capability`, `oracle_26ai_secondary_capabilities`, `feature_usage_pattern`, and `why_this_oracle_feature_fits`
- first-iteration experience with `first_scene_goal`, `first_interaction`, `first_decision_point`, and `first_oracle_evidence`
- `Use Your Own Data` dataset-admin acceptance criteria when demo data is replaceable
...
- treating Redwood/JET polish or `Use Your Own Data` as a later add-on
