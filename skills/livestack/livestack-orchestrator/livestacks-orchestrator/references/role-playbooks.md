...
- Oracle 26ai primary and secondary capability strategy, feature usage pattern, and why the selected Oracle features fit the pain point
...
- dynamic scene architecture with Command Center, Data Foundation, and the right-sized set of capability-backed workflow scenes
...
- screen map
- story mode with dynamic scene architecture and a right-sized `scene_count_target`
...
- scene manifest contract for navigation labels, guide order, screenshot names, primary CTAs, per-scene Oracle feature chips, and Oracle Internals payloads
- Scene Experience Contract for each scene: scene-specific component/page module, distinct interaction pattern, local state, visible domain objects, real Oracle evidence per scene, workflow handoff, and screenshot caption proof
- repeatable app shell anatomy with Oracle logo/name lockup, left-side scene rail, top-right utilities, and right-side scene-aware Oracle Internals
- Oracle Internals scene contract: each scene owns `What's Happening`, capability badges, SQL/PLSQL snippets, architecture/data-flow diagram boxes, security/governance callouts, and live evidence or dynamic state
- navigation model
- feature-to-scene mapping co-authored with the database role so the story shape matches the chosen Oracle capabilities
- operator-only dataset admin journey when demo data must be replaceable, using a persistent top-right masthead `Use Your Own Data` utility that opens an overlay dataset manager unless the user explicitly requests another pattern
- persistent clickable/collapsible, scene-aware Oracle Internals control or database X-Ray journey for the key screens
...
- app navigation is on the left, uses `oj-fwk-icon-*` glyphs, and shows the Oracle features demonstrated by each scene through feature chips or equivalent tags
- every scene is visually and behaviorally distinct, with its own component/page module, interaction pattern, state, domain objects, Oracle evidence, and handoff rather than a repeated generic stage
- Oracle Internals is a right-side persistent rail or equivalent panel populated by active-scene registration, not a static card inside the main content
- Oracle Internals content changes per scene and includes a real SQL/PLSQL snippet, diagram/data-flow boxes, governance callout, and live state; when `Use Your Own Data` is open, the rail switches to dataset governance and admin-boundary evidence
- the UI looks polished and premium enough for an Oracle customer walkthrough in the first iteration
- replaceable demo-data solutions include a credible top-right `Use Your Own Data` masthead utility plus dataset upload/admin and restore experience
...
- capability-led operator application scaffolding that consumes Oracle 26ai feature usage pattern, `story_mode`, dynamic scene architecture, and `scene_count_target`
...
- Oracle Internals or database X-Ray contract, including how scene-level evidence is surfaced through a persistent clickable/collapsible active-scene control
...
- the generated scenes satisfy the Scene Experience Contract instead of sharing one repeated stage and primary action
...
- the app exposes an Oracle Internals panel or database X-Ray mode backed by real routes, SQL, packages, diagrams, governance callouts, and runtime evidence rather than generic metadata
