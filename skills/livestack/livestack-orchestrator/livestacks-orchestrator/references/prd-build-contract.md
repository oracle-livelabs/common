...
- Treat Oracle 26ai feature strategy as the first design decision. Record `oracle_26ai_primary_capability`, `oracle_26ai_secondary_capabilities`, `feature_usage_pattern`, `why_this_oracle_feature_fits`, `evidence_surface_per_scene`, and `oracle_internals_scene_payload` before finalizing UI scenes.
...
- Derive story mode and dynamic scene architecture from the workflow shape, operator decision points, inferred Oracle feature breadth, evidence needs, guide/demo pacing, and customer-data onboarding requirements before UI and application scaffolding begins. Do not force a fixed scene count.
...
- Treat `Use Your Own Data` as a first-iteration requirement whenever demo data is replaceable.
...
- `## Story Architecture`: define `story_mode`, a right-sized `scene_count_target`, `primary_user_loop`, `scene_sequence`, `oracle_protagonist_beats`, `primary_cta_path`, and `dataset_role`
...
- `## Oracle 26ai Capability Strategy`: name primary/secondary Oracle features, rejected alternatives, and why the selected feature set is the minimum credible fit
- `## Feature Usage Pattern`: name the pattern being proven, such as semantic matching, RAG, safe NL-to-SQL, guided agent workflow, explainable recommendation, optimization, anomaly/risk scoring, or a documented non-AI Oracle pattern
...
- how the dynamic scene architecture proves the selected Oracle 26ai feature usage patterns without forcing a fixed scene count
- how the Scene Experience Contract is met: scene-specific components/page modules, distinct interaction patterns, scene-local state, visible domain objects, real Oracle evidence per scene, workflow handoffs, and screenshot captions that prove visual and behavioral distinction
- how the clickable/collapsible Oracle Internals control is populated by each active scene, including `What's Happening`, capability badges, SQL/PLSQL snippets, architecture/data-flow boxes, governance/security callouts, and live evidence or dynamic state
...
- how `Use Your Own Data` is surfaced and wired when demo data is replaceable, including the dataset-governance Oracle Internals state shown while the overlay is open
