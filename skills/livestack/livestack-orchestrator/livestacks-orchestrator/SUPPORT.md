...
- `python3`
- public GitHub network access for automatic self-update checks
...
- Oracle AI Database 26ai feature strategy as the first design decision
- Command Center, Data Foundation, and a right-sized set of dynamic capability-backed scenes
- a persistent clickable/collapsible, scene-aware Oracle evidence surface populated by the active scene
- an interactive `operator_workbench` first screen for sparse briefs unless the working PRD justifies another story mode
- top-right `Use Your Own Data` as a first-iteration dataset-admin flow when demo data is replaceable
...
- final A+ grading with capability-led operator application evidence and golden-core runtime parity through `scripts/grade_livestack_bundle.py`
