#!/usr/bin/env python3
"""Install the bundled clean-zip snapshot into $CODEX_HOME/skills if missing."""
...
def materialize_skill_metadata(destination: Path) -> None:
    ...
    if live_skill.exists():
        return
    raise SystemExit(
        f"Bundled {SKILL_NAME} snapshot is missing {BUNDLED_SKILL_FILE} or {LIVE_SKILL_FILE}: {destination}"
    )
...
def install_bundled_skill(dest_root: Path, force: bool) -> Path:
    ...
    if not source.exists():
        raise SystemExit(f"Bundled {SKILL_NAME} snapshot not found: {source}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help=f"Replace an existing bundled-install destination named {SKILL_NAME}.",
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help=f"Only report whether {SKILL_NAME} is already available; do not install it.",
    )
