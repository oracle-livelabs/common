...
Document the application as a capability-led operator application in premium Oracle JET / Redwood terms. Use `$redwood-creator` as the app-UI lane source of truth when it is available.
...
- Replaceable demo-data solutions include a top-right `Use Your Own Data` masthead utility plus template download, validate preview, upload status, active dataset state, and demo restore.
- The application includes a persistent clickable/collapsible, scene-aware Oracle Internals panel or database X-Ray mode tied to real Oracle behavior.
""",
