#!/usr/bin/env python3
"""Install the bundled redwood-creator snapshot into $CODEX_HOME/skills if missing."""
...
def bundled_skill_root() -> Path:
    return Path(__file__).resolve().parents[1] / "assets" / "bundled" / "redwood-creator"
...
def find_existing_skill(root: Path) -> Path | None:
    if not root.exists():
        return None

    top_level = root / LIVE_SKILL_FILE
    if top_level.exists() and parse_skill_name(top_level) == "redwood-creator":
    ...
    for child in sorted(root.iterdir()):
        ...
        if skill_file.exists() and parse_skill_name(skill_file) == "redwood-creator":
            return child
    return None
...
def materialize_skill_metadata(destination: Path) -> None:
    ...
    if live_skill.exists():
        return
    raise SystemExit(
        f"Bundled redwood-creator snapshot is missing {BUNDLED_SKILL_FILE} or {LIVE_SKILL_FILE}: {destination}"
    )
...
def install_bundled_skill(dest_root: Path, force: bool) -> Path:
    ...
    if not source.exists():
        raise SystemExit(f"Bundled redwood-creator snapshot not found: {source}")

    destination = dest_root / "redwood-creator"
...
def main() -> int:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Replace an existing bundled-install destination named redwood-creator.",
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help="Only report whether redwood-creator is already available; do not install it.",
    )
    args = parser.parse_args()

    dest_root = Path(args.dest_root).expanduser() if args.dest_root else skills_root()
    existing = find_existing_skill(dest_root)
    if existing:
        print(existing)
        return 0
