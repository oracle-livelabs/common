...
PLACEHOLDER_STRINGS = (
    ...
    "Capability-Led Operator Application Starter",
    "starter_contract",
    "replace-with-",
    "replace-with-oracle",
)
...
EXPECTED_GOLDEN_BASELINE = "capability-led-operator-application-core"
...
CAPABILITY_LED_PRD_KEYS = (
    "oracle_26ai_primary_capability",
    "oracle_26ai_secondary_capabilities",
    "feature_usage_pattern",
    "why_this_oracle_feature_fits",
    "evidence_surface_per_scene",
    "oracle_internals_scene_payload",
)
...
ORACLE_INTERNALS_WHATS_HAPPENING_HINTS = (
    "what's happening",
    "what is happening",
    "whats happening",
)

ORACLE_INTERNALS_BADGE_HINTS = (
    "featurebadge",
...
ORACLE_INTERNALS_DIAGRAM_HINTS = (
    "diagrambox",
    "oracle-diagram",
    "diagram box",
    "data flow",
...
ORACLE_INTERNALS_GOVERNANCE_HINTS = (
    "governancecallout",
    "governance boundary",
    "security/governance",
...
ORACLE_INTERNALS_DATASET_HINTS = (
    "datasetoverlayopen",
    "dataset internals",
    "dataset governance",
    "use your own data",
...
def contains_claim_token(text: str, token: str) -> bool:
    """Match feature-claim tokens without substring false positives."""
    text = text.lower()
    token = token.lower()
...
SCENE_COMPONENT_RE = re.compile(
    r"\b(?:function|const)\s+([A-Z][A-Za-z0-9]*(?:Scene|Page))\b"
)

SCENE_LOCAL_STATE_RE = re.compile(
    r"\[\s*([A-Za-z_][A-Za-z0-9_]*)\s*,\s*set[A-Za-z_][A-Za-z0-9_]*\s*\]\s*=\s*(?:React\.)?useState\s*\("
)

SCENE_STRING_VALUE_RE = re.compile(
    r"\b(?P<key>interactionPattern|sceneType|evidenceObject|handoffTargets)\s*:\s*[\"'](?P<value>[^\"']+)[\"']"
)

SCENE_LOCAL_STATE_EXCLUDES = {
    "activeSceneId",
    "datasetOverlayOpen",
    "oraclePanelCollapsed",
    "health",
    "evidence",
    "actionLog",
}

SCENE_INTERACTION_CATEGORY_HINTS = {
    "queue_triage": ("queue", "triage", "decision-queue", "triage-board", "status lane"),
    "schema_lineage": ("schema", "lineage", "schema-map", "schema-node", "data foundation"),
    "ranked_evidence": ("ranked evidence", "evidence-results", "evidence-result", "source attribution"),
    "api_workbench": ("api workbench", "json document api", "method selector", "response inspector"),
    "report_runner": ("report runner", "sql reports", "report selector", "bar visualization"),
    "dataset_manager": ("dataset manager", "dataset-overlay", "validate", "upload", "restore-demo"),
    "architecture_comparison": ("architecture", "before 26ai", "with 26ai", "comparison"),
    "governed_action": ("action-selector", "action-option", "audit ledger", "publish"),
    "map_or_graph": ("map", "graph", "network", "spatial"),
    "chat_or_assistant": ("chat", "assistant", "conversation"),
}

SCENE_DOMAIN_CATEGORY_HINTS = {
    "selected_record": ("selecteddecisionitem", "selectedrequest", "selectedrecord", "selected item"),
    "domain_card": ("domain-object-card", "request-card", "feature-card", "decision-card"),
    "schema_object": ("schemaobject", "schema object", "schema-node", "oracle object"),
    "evidence_object": ("evidenceobject", "evidence object", "evidence-result", "source attribution"),
    "business_entity": ("customer", "feature request", "request", "vote", "comment", "tag", "case-", "item_id"),
    "action_object": ("selectedaction", "activeaction", "action-option", "decision code"),
}

SCENE_SCREENSHOT_CAPTION_HINTS = {
    "queue_triage": ("queue", "triage", "status lane", "decision panel", "request queue"),
    "schema_lineage": ("schema", "lineage", "oracle object", "duality view", "dataset state"),
    "ranked_evidence": ("ranked evidence", "source attribution", "vector search", "evidence workbench"),
    "api_workbench": ("api workbench", "json document api", "post payload", "ords response", "method selector"),
    "report_runner": ("sql reports", "report selector", "bar visualization", "report runner"),
    "dataset_manager": ("dataset manager", "template", "validate", "upload", "restore-demo"),
    "architecture_comparison": ("architecture shift", "before 26ai", "with 26ai", "comparison"),
    "governed_action": ("action", "audit", "publish", "approval", "handoff"),
    "map_or_graph": ("map", "graph", "network", "spatial"),
    "chat_or_assistant": ("chat", "assistant", "conversation"),
}

SCENE_MANIFEST_HINTS = (
    "sceneManifest",
    "scene-manifest",
    "SCENE_MANIFEST",
    "guideSlug",
...
FEATURE_CHIP_HINTS = (
    "feature-chip",
    "oracleFeatures",
    "Oracle feature chips",
)

ORACLE_PANEL_REGISTRATION_HINTS = (
    "RegisterOraclePanel",
    "registerSceneEvidence",
    "useOraclePanel",
    "oracle-panel-registration",
)

RIGHT_ORACLE_PANEL_HINTS = (
    "right-oracle-panel",
    "RightOraclePanel",
)
...
def iter_frontend_source_files(root: Path) -> list[Path]:
    frontend_root = root / "stack" / "frontend" / "src"
...
def read_frontend_sources(root: Path) -> dict[str, str]:
    sources: dict[str, str] = {}
    for path in iter_frontend_source_files(root):
        relative_path = str(path.relative_to(root)).replace("\\", "/")
        sources[relative_path] = path.read_text(encoding="utf-8", errors="ignore")
    return sources


def category_matches(text: str, categories: dict[str, tuple[str, ...]]) -> set[str]:
    lower = text.lower()
    return {
        category
...
def scene_experience_summary(root: Path) -> dict[str, object]:
    sources = read_frontend_sources(root)
    combined = "\n".join(sources.values())
    app_text = sources.get("stack/frontend/src/App.jsx", "")
    lower_app = app_text.lower()

    component_names = set(SCENE_COMPONENT_RE.findall(combined))
    page_module_paths = {
        path
        for path in sources
        if "/pages/" in path
        or re.search(r"(?:Scene|Page)\.(?:jsx|tsx|js|ts)$", path)
    }
    state_names = set(SCENE_LOCAL_STATE_RE.findall(combined)) - SCENE_LOCAL_STATE_EXCLUDES
    interaction_values = {
        match.group("value").strip().lower()
        for match in SCENE_STRING_VALUE_RE.finditer(combined)
        if match.group("key") == "interactionPattern"
    }
    workbench_tokens = set(
        re.findall(
            r"\b[a-z0-9_-]*(?:workbench|inspector|runner|triage|lineage|queue|map|graph|chat|report|api|schema|evidence|action|selector|board)[a-z0-9_-]*\b",
            combined.lower(),
        )
    )
    interaction_categories = category_matches(combined, SCENE_INTERACTION_CATEGORY_HINTS)
    domain_categories = category_matches(combined, SCENE_DOMAIN_CATEGORY_HINTS)
    manifest_scene_count = len(re.findall(r"\bid\s*:\s*[\"'][a-z0-9_-]+[\"']", combined))

    has_scene_specific_components = len(component_names) + len(page_module_paths) >= 3
    has_distinct_interaction_patterns = (
        len(interaction_values) >= 3
        and (len(interaction_categories) >= 3 or len(workbench_tokens) >= 3)
        and ("data-interaction-pattern" in combined or len(workbench_tokens) >= 3)
    )
    has_scene_local_state = len(state_names) >= 3
    has_domain_objects = len(domain_categories) >= 3
    has_oracle_evidence_per_scene = all(
        token in combined
        for token in ("evidenceObject", "oracleInternalsScenePayload")
    ) and (
        "/api/oracle/evidence?scene=" in combined
        or "scene.id" in combined
        or "activeScene.id" in combined
    )
    has_workflow_handoffs = (
        "handoffTargets" in combined
        and (
            "goToScene" in app_text
            or "onNavigate" in app_text
            or "handoff" in lower_app
        )
    )
    generic_repeated_scene_stage = (
        "scene-stage" in lower_app
        and "runprimaryaction" in lower_app
        and (
            "renderscene" not in lower_app
            or len(component_names) < 3
            or len(interaction_categories) < 3
        )
    )

    return {
        "frontend_found": bool(sources),
        "component_names": sorted(component_names),
        "page_module_paths": sorted(page_module_paths),
        "state_names": sorted(state_names),
        "interaction_values": sorted(interaction_values),
        "interaction_categories": sorted(interaction_categories),
        "workbench_tokens": sorted(workbench_tokens),
        "domain_categories": sorted(domain_categories),
        "manifest_scene_count": manifest_scene_count,
        "has_scene_specific_components": has_scene_specific_components,
        "has_distinct_interaction_patterns": has_distinct_interaction_patterns,
        "has_scene_local_state": has_scene_local_state,
        "has_domain_objects": has_domain_objects,
        "has_oracle_evidence_per_scene": has_oracle_evidence_per_scene,
        "has_workflow_handoffs": has_workflow_handoffs,
        "generic_repeated_scene_stage": generic_repeated_scene_stage,
    }


def screenshot_caption_experience_summary(root: Path) -> dict[str, object]:
    json_path = root / "output" / "guide-screenshots" / "inventory.json"
    if not json_path.exists():
        return {"available": False, "passed": False, "entries": 0}
    ...
    for entry in inventory:
        ...
        caption_text = " ".join(
            ...
            for key in ("caption", "alt", "note")
            if str(entry.get(key, "")).strip()
        )
        if not caption_text:
            continue
        caption_texts.append(caption_text)
        if not category_matches(caption_text, SCENE_SCREENSHOT_CAPTION_HINTS):
            generic_entries += 1

    normalized_captions = {re.sub(r"\s+", " ", caption.lower()).strip() for caption in caption_texts}
    categories = category_matches("\n".join(caption_texts), SCENE_SCREENSHOT_CAPTION_HINTS)
    expected_distinct = min(3, len(caption_texts))
    repeated_captions = len(caption_texts) >= 3 and len(normalized_captions) < expected_distinct
    passed = (
        len(caption_texts) >= 3
        and not repeated_captions
        and generic_entries == 0
        and len(categories) >= expected_distinct
    )

    return {
        "available": True,
        "passed": passed,
        "entries": len(caption_texts),
        "categories": sorted(categories),
        "generic_entries": generic_entries,
        "repeated_captions": repeated_captions,
    }
...
class Validator:
    ...
    def validate_canonical_core_files(self) -> None:
        for relative_path, canonical_path in CANONICAL_CORE_FILES.items():
            ...
            if actual != expected:
                self.add(
                    relative_path,
                    "file must match the capability-led operator application runtime baseline; move industry, pain-point, and story variation into app/database/config overlays instead of changing this core file",
                )
    ...
    def validate_app_contract(self) -> None:
        ...
        oracle_internals_clickable_found = False
        oracle_internals_scene_aware_found = False
        oracle_internals_whats_happening_found = False
        oracle_internals_badges_found = False
        oracle_internals_sql_found = False
        oracle_internals_diagram_found = False
        oracle_internals_governance_found = False
        oracle_internals_live_found = False
        oracle_internals_dataset_state_found = False
        scene_manifest_found = False
        brand_lockup_found = False
        left_scene_navigation_found = False
        feature_chips_found = False
        oracle_panel_registration_found = False
        right_oracle_panel_found = False
        command_center_runtime_found = False
        data_foundation_runtime_found = False
        ...
        for path in self.iter_runtime_text_files():
            ...
            lower = text.lower()
            lower_path = relative_path.lower()
            rich_oracle_candidate = (
                path.suffix.lower() != ".css"
                and (
                    "oracleinternalscontent" in lower_path
                    or "registeroraclepanel" in lower
                    or "rightoraclepanel" in lower_path
                    or "oracle-internals-section" in lower
                    or "oracle-sql-block" in lower
                    or "oracle-diagram" in lower
                    or "governancecallout" in lower
                    or "liveevidence" in lower
                )
            )
            non_style_lower = lower if rich_oracle_candidate else ""
            ...
            if "upload your own data" in lower or "use your own data" in lower:
                upload_data_runtime_found = True
            if any(hint in text for hint in SCENE_MANIFEST_HINTS):
                scene_manifest_found = True
            if any(hint in text for hint in BRAND_LOCKUP_HINTS):
                brand_lockup_found = True
            if any(hint in text for hint in LEFT_SCENE_NAV_HINTS):
                left_scene_navigation_found = True
            if any(hint in text for hint in FEATURE_CHIP_HINTS):
                feature_chips_found = True
            if any(hint in text for hint in ORACLE_PANEL_REGISTRATION_HINTS):
                oracle_panel_registration_found = True
            if any(hint in text for hint in RIGHT_ORACLE_PANEL_HINTS):
                right_oracle_panel_found = True
            if "command center" in lower:
                command_center_runtime_found = True
            if "data foundation" in lower:
                data_foundation_runtime_found = True
            if "Oracle Internals" in text or "Database X-Ray" in text:
                oracle_internals_runtime_found = True
                if (
                    ("button" in lower or "<oj-button" in lower)
                    and ("collapsed" in lower or "collapse" in lower or "aria-expanded" in lower)
                ):
                    oracle_internals_clickable_found = True
                if any(
                    hint in lower
                    for hint in (
                        "activescene",
                    ...
                    oracle_internals_scene_aware_found = True
            if any(hint in non_style_lower for hint in ORACLE_INTERNALS_WHATS_HAPPENING_HINTS):
                oracle_internals_whats_happening_found = True
            if any(hint in non_style_lower for hint in ORACLE_INTERNALS_BADGE_HINTS):
                oracle_internals_badges_found = True
            if any(hint in non_style_lower for hint in ORACLE_INTERNALS_SQL_HINTS):
                oracle_internals_sql_found = True
            if any(hint in non_style_lower for hint in ORACLE_INTERNALS_DIAGRAM_HINTS):
                oracle_internals_diagram_found = True
            if any(hint in non_style_lower for hint in ORACLE_INTERNALS_GOVERNANCE_HINTS):
                oracle_internals_governance_found = True
            if any(hint in non_style_lower for hint in ORACLE_INTERNALS_LIVE_HINTS):
                oracle_internals_live_found = True
            if any(hint in non_style_lower for hint in ORACLE_INTERNALS_DATASET_HINTS):
                oracle_internals_dataset_state_found = True
        ...
        if not upload_data_runtime_found:
            self.add("stack/frontend", "runtime UI must expose the persistent top-right `Use Your Own Data` dataset-admin entry")
        else:
            for route, found in dataset_route_found.items():
                if not found:
                    self.add(
                        "stack",
                        f"`Use Your Own Data` is visible but the runtime is missing dataset API route `{route}`",
                    )
        if not scene_manifest_found:
            self.add(
                "stack/frontend/src/sceneManifest.js",
                "frontend must include a scene metadata manifest for navigation, guide/screenshot order, feature chips, primary CTAs, and Oracle Internals payloads",
            )
        if not brand_lockup_found:
            self.add("stack/frontend", "runtime UI must expose an Oracle logo/name lockup in the app shell")
        if not left_scene_navigation_found:
            self.add("stack/frontend", "runtime UI must use left-side scene navigation, not only centered tabs or buttons")
        if left_scene_navigation_found and jet_icon_count < 4:
            self.add("stack/frontend", "left-side scene navigation must use `oj-fwk-icon-*` glyphs for multiple scenes")
        if not feature_chips_found:
            self.add("stack/frontend", "scene navigation must show per-scene Oracle feature chips or equivalent feature tags")
        if not right_oracle_panel_found:
            self.add("stack/frontend", "Oracle Internals must render as a right-side rail or panel, not only a content-card section")
        if not oracle_panel_registration_found:
            self.add(
                "stack/frontend",
                "Oracle Internals must use active-scene panel registration or equivalent scene evidence wiring",
            )
        ...
        if not oracle_internals_clickable_found:
            self.add(
                "stack/frontend",
                "Oracle Internals must be a persistent clickable/collapsible control, not only a static evidence section",
            )
        if not oracle_internals_scene_aware_found:
            self.add(
                "stack/frontend",
                "Oracle Internals must be scene-aware and populated by the active scene",
            )
        oracle_internals_contract = (
            (oracle_internals_whats_happening_found, "include a per-scene `What's Happening` explanation"),
            (oracle_internals_badges_found, "include color-coded Oracle capability badges"),
            (oracle_internals_sql_found, "include real SQL or PL/SQL snippets"),
            (oracle_internals_diagram_found, "include diagram boxes that show architecture or data flow"),
            (oracle_internals_governance_found, "include scene-specific security or governance callouts"),
            (oracle_internals_live_found, "include live evidence or dynamic state from the running scene"),
            (oracle_internals_dataset_state_found, "include a Use Your Own Data or dataset-admin internals state"),
        )
        for passed, detail in oracle_internals_contract:
            if oracle_internals_runtime_found and not passed:
                self.add("stack/frontend", f"Oracle Internals must {detail}")
        if not command_center_runtime_found:
            self.add("stack/frontend", "capability-led operator app must include a Command Center surface")
        if not data_foundation_runtime_found:
            self.add("stack/frontend", "capability-led operator app must include a Data Foundation surface")
        ...
        scene_summary = scene_experience_summary(self.root)
        if scene_summary["frontend_found"]:
            if scene_summary["generic_repeated_scene_stage"]:
                self.add(
                    "stack/frontend/src/App.jsx",
                    "Scene Experience Contract violation: repeated `scene-stage` with one `runPrimaryAction` cannot replace scene-specific components, distinct interaction patterns, scene-local state, domain objects, Oracle evidence per scene, and workflow handoffs",
                )
            scene_contract_checks = (
                (
                    scene_summary["has_scene_specific_components"],
                    "Scene Experience Contract requires a scene-specific component or page module for each major scene",
                ),
                (
                    scene_summary["has_distinct_interaction_patterns"],
                    "Scene Experience Contract requires distinct interaction patterns, not one repeated generic stage",
                ),
                (
                    scene_summary["has_scene_local_state"],
                    "Scene Experience Contract requires scene-local state such as selected records, filters, evidence, or action choices",
                ),
                (
                    scene_summary["has_domain_objects"],
                    "Scene Experience Contract requires visible domain objects per scene",
                ),
                (
                    scene_summary["has_oracle_evidence_per_scene"],
                    "Scene Experience Contract requires real Oracle evidence per scene",
                ),
                (
                    scene_summary["has_workflow_handoffs"],
                    "Scene Experience Contract requires workflow handoffs between scenes",
                ),
            )
            for passed, message in scene_contract_checks:
                if not passed:
                    self.add("stack/frontend", message)
    ...
    def validate_docs_alignment(self) -> None:
        ...
        if self.exists(working_prd_path):
            ...
            for key in AI_PRD_KEYS:
                if key not in text:
                    self.add(working_prd_path, f"working PRD must declare `{key}`")
            for key in CAPABILITY_LED_PRD_KEYS:
                if key not in text:
                    self.add(working_prd_path, f"working PRD must declare `{key}`")
        ...
        if self.exists(ui_path):
            ...
            if "Upload Your Own Data" not in text and "Use Your Own Data" not in text:
                self.add(ui_path, "UI concept must describe the top-right `Use Your Own Data` first-iteration entry")
            ...
            if "scene manifest" not in lower_text and "sceneManifest" not in text:
                self.add(ui_path, "UI concept must describe the scene manifest that drives navigation, feature tags, guide order, screenshots, and Oracle Internals payloads")
            if "left-side scene navigation" not in lower_text and "left scene" not in lower_text and "scene rail" not in lower_text:
                self.add(ui_path, "UI concept must describe the left-side scene rail/navigation pattern")
            if "oracle logo" not in lower_text and "oracle lockup" not in lower_text and "logo/name lockup" not in lower_text:
                self.add(ui_path, "UI concept must describe the Oracle logo/name lockup in the app shell")
            if "feature chip" not in lower_text and "feature tag" not in lower_text:
                self.add(ui_path, "UI concept must describe per-scene Oracle feature chips or tags")
            if "right-side" not in lower_text and "right side" not in lower_text and "right rail" not in lower_text:
                self.add(ui_path, "UI concept must place Oracle Internals in a right-side rail or equivalent persistent panel")
            ...
            if "dynamic scene architecture" not in lower_text and "dynamic_scene_architecture" not in text:
                self.add(ui_path, "UI concept must describe dynamic scene architecture")
            if "scene experience contract" not in lower_text:
                self.add(ui_path, "UI concept must describe the Scene Experience Contract for scene-specific components, distinct interactions, local state, domain objects, per-scene Oracle evidence, workflow handoffs, and screenshot captions")
            if "command center" not in lower_text:
                self.add(ui_path, "UI concept must include a Command Center surface")
            if "data foundation" not in lower_text:
                self.add(ui_path, "UI concept must include a Data Foundation surface")
            ...
            for key in CAPABILITY_LED_PRD_KEYS:
                if key not in text:
                    self.add(ui_path, f"UI concept must declare `{key}`")
            if "clickable" not in lower_text or "collapsible" not in lower_text or "scene-aware" not in lower_text:
                self.add(ui_path, "UI concept must define Oracle Internals as clickable, collapsible, and scene-aware")
            if "bottom-left" in lower_text or "bottom left" in lower_text:
                self.add(ui_path, "UI concept must use the top-right masthead pattern for `Use Your Own Data`, not a bottom-left CTA")
    ...
    def validate_guide_contract(self) -> None:
        ...
        byod_candidates: list[tuple[str, str]] = []

        for path in (
            guide_root / "introduction" / "introduction.md",
            guide_root / "download-livestack" / "download-livestack.md",
            *([guide_root / "conclusion" / "conclusion.md"] if (guide_root / "conclusion" / "conclusion.md").exists() else []),
            *[guide_root / relative_path for relative_path in scene_paths],
            ...
            if "/scene-" in relative_path:
                ...
                if not any(token in lower_text for token in GUIDE_OUTCOME_HINTS):
                    self.add(relative_path, "scene runbook lab must state the expected business outcome or signal")
                if GUIDE_BYOD_PATTERN.search(relative_path) or GUIDE_BYOD_PATTERN.search(text):
                    byod_candidates.append((relative_path, text))

        self.validate_guide_byod_scene(byod_candidates)
    ...
    def validate_guide_byod_scene(self, byod_candidates: list[tuple[str, str]]) -> None:
        if not byod_candidates:
            self.add("guide", "guide must include a full Use Your Own Data or dataset workflow scene")
            return

        combined = "\n".join(text.lower() for _relative_path, text in byod_candidates)
        for alternatives in GUIDE_BYOD_REQUIRED_TERMS:
    ...
    def validate_screenshot_inventory(self) -> None:
        ...
        caption_summary = screenshot_caption_experience_summary(self.root)
        if inventory and caption_summary.get("available") and not caption_summary.get("passed"):
            self.add(
                json_path,
                "Scene Experience Contract requires screenshot captions that prove scenes are visually and behaviorally different",
            )
    ...
    def validate_oracle_evidence_surface(self) -> None:
        ...
        vector_claimed = any(contains_claim_token(combined, token) for token in VECTOR_CLAIM_TOKENS)
        if vector_claimed:
