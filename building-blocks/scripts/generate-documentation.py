...
def write_task_manifest():
    ...
    output = add_line(output,
                      '         "filename": "' + relpath_blocks + '\\how-to-author-with-blocks\\how-to-author-with-blocks.md"')
    output = add_line(output, '     },')
    output = add_line(output, '     {')
    ...
    output = add_line(output, '         "type": "freetier",')
    output = add_line(output,
                      '         "filename": "' + relpath_blocks + '\\how-to-author-with-blocks\\creating-blocks-tasks.md"')
    output = add_line(output, '     },')
    output = add_line(output, '     {')
    output = add_line(output, '         "title": "List of Building Blocks and Tasks",')
    output = add_line(output, '         "type": "freetier",')
    output = add_line(output,
                      '         "filename": "' + relpath_blocks + '\\how-to-author-with-blocks\\all-blocks-tasks.md"')
    output = add_line(output, '     },')
...
def write_toc():
    ...
    output = add_line(output,
                      '| setup | [Add Workshop Utilities](' + relpath_blocks + '\\workshop\\freetier\\index.html?lab=add-workshop-utilities) |  ' + relpath_blocks + ' \\setup\\add-workshop-utilities.md| Utilities for adding data sets and users |')
    ...
    for t in blocks:
        ...
        output = add_line(output,
                          "| " + t.cloud_service + " | " + this_anchor + " | " + t.path + " | " + this_description + " |")
    ...
    for t in tasks:
        this_name = t.md_name if not t.name else t.name
        this_anchor = relpath_blocks + '\\how-to-author-with-blocks\\workshop\\index.html?lab=' + t.cloud_service + '#' + re.sub(
            '[^0-9a-zA-Z]+', '', this_name)
        this_anchor = "[" + this_name + "](" + this_anchor + ")"
        this_description = t.description if t.description else " "
        output = add_line(output,
