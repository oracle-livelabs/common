...
1. a redesigned app route for Guided Path, Toolkit, Step by Step Guide, and search
...
* `index.html`
  ...
  Child routes live inside the same page as modes: Home, Guided Path, Toolkit, Step by Step Guide, and Search.
...
* `workshops/author-guide/manifest.json`
  Canonical local page order used by the redesigned Step by Step Guide cards.
  The visible `Open Step by Step Guide` actions use this order to build live original guide links with the matching `?lab=<page-id>`.
  Anything not listed here stays outside the active redesigned Step by Step Guide and local focused variants even if legacy files still exist elsewhere in the repo.
...
* `assets/js/guide-content.js`
  ...
  This file is no longer the canonical source for the Step by Step Guide route.
* `assets/js/author-guide-app.js`
  Redesigned app behavior, routing, search, and manifest-driven Step by Step Guide rendering.
...
The legacy section-based wrappers were removed from the LiveLabs payload. The active local markdown entry points are the focused variants under `workshops/variants/`; the Step by Step Guide entry points redirect to the live original guide.
...
* If the redesigned Step by Step Guide needs to reflect the new order or metadata, update the manifest-driven behavior in `assets/js/author-guide-app.js`.
